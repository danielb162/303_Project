...
public class Simulator {
      ...
      for ( int y = 0; y < max_y; y++ ){
        for ( int x = 0; x < max_x; x++ ){
          if ( item_arr[y][x] != null && item_arr[y][x].getToken() == 'A' && item_arr[y][x].getMode() == false ) {
            ...
            System.out.println("Char is: " + dir);
            switch ( dir ) {
                  ...
                  while ( col_count < max_x ) {
                    if ( item_arr[y][col_count] == null ) {
                      ...
                      while ( count > 0 ) {
                        ...
                        if ( count == 1 ) item_arr[y][col_count].setMode(true); // to prevent further, autonomous movement
  ...
  // Ints ssed to populate item_arr, can be changed by TA's to make testing easier
  ...
  public static void main(String[] args) throws WorldItemException {
    // Some variables we need to setup
